...
class MjpegBroadcaster:
    ...
    async def force_shutdown(self, *, force: bool = False) -> None:
        ...
        Note the suppressed path deliberately leaves ``_stopped`` False and the
        registry entry in place, so ``get_or_create_broadcaster`` keeps handing
        out *this* broadcaster rather than minting a replacement — the #2521
        predecessor gate is never engaged and the printer only ever sees one
        socket.

        ``force=True`` overrides the pin and tears down anyway. Only app
        shutdown uses it: there, honouring a pin would leave the pump and its
        upstream socket running after the registry has been cleared, with
        nothing left to release them.
        """
        async with self._lock:
            if self._pinned_subscribers and not force:
                logger.info(
...
async def shutdown_all_broadcasters() -> None:
    ...
    Uses ``force=True`` so a pinned subscriber (an in-flight Sentry recording)
    cannot keep a pump and its upstream socket alive here. Every other caller
    respects the pin; this one cannot, because the registry is cleared on the
    line above and there would be nothing left holding a reference to release
    the socket once the recording ended.
    """
    async with _registry_lock:
        bcs = list(_broadcasters.values())
        _broadcasters.clear()
    await asyncio.gather(*(bc.force_shutdown(force=True) for bc in bcs), return_exceptions=True)
