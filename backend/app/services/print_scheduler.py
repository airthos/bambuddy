...
class PrintScheduler:
    ...
    async def _start_print(self, db: AsyncSession, item: PrintQueueItem):
        ...
        # Farm post-process script. Runs *after* upstream's #422 snippet injection
        # so it operates on the already-injected plate gcode (both rewrite the same
        # `Metadata/plate_{N}.gcode` inside the 3MF and our script recomputes the
        # md5, so chaining is safe).
        if item.script_processing:
            ...
            if processed_path is not None:
                ...
                # The post-processor replaces the stock end sequence with the bed
                # cooldown + push-off cycle, so this print ends with our own End
                # G-code even when no #422 snippet was configured. That is exactly
                # the condition #2547 cares about: the part is swept off the plate
                # before `gcode_state=FINISH`, so a live grab at FINISH photographs
                # an empty bed. Flag it so the finish photo is sourced from the
                # in-print frame bank instead (#1867).
                end_gcode_injected = True
        ...
        if not uploaded:
            ...
            # A deadline expiry is not transient: the link sustained less than the
            # floor rate for an entire deadline, which is why `with_ftp_retry`
            # refuses to retry it (#2529). Spending another full deadline to learn
            # the same thing would block the queue, so fail straight through rather
            # than consuming a dispatch attempt.
            if upload_error is None:
                # Share upstream's persisted per-item budget (#2555) rather than a
                # second in-memory counter: an item that has bounced back to
                # 'pending' three times should stop, whether the cause was a
                # pre-dispatch FTP failure or a printer that accepted the file and
                # never started. Persisted, so the count survives a restart.
                item.dispatch_attempts = (item.dispatch_attempts or 0) + 1
                logger.error(
                    "Queue item %s: FTP upload failed (attempt %d/%d) - printer=%s, model=%s, ip=%s. "
            ...
            if upload_error is None and item.dispatch_attempts < DISPATCH_MAX_ATTEMPTS:
            ...
            # Retries exhausted (or a non-retryable deadline expiry) — fail for real.
            error_msg = upload_error or (
                f"Failed to upload file to printer after {item.dispatch_attempts} attempts. "
